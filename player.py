...
class MusicPlayer:
    ...
    async def _start_stream(self, chat_id: int):
        ...
        try:
            ...
            await self._next(chat_id)  # skip to next
    ...
    async def _monitor_stream(self, chat_id: int, msg_id: int):
        while True:
            ...
            if self.paused.get(chat_id, False):
                continue
            # Check if finished – we rely on callback or duration. 
            # Simpler: we'll check if audio stops.
            # Just assume it finishes after duration + 2 seconds.
