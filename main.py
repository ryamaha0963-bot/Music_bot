    ...
    if action == "pause":
        ...
        await cb.message.edit_text(
            ...
            reply_markup=control_buttons(
                chat_id,
                not player.paused.get(chat_id, False),
                player.loop.get(chat_id, False)
            )
        )
        await cb.answer()
        ...
        await cb.message.edit_text(
            ...
            reply_markup=control_buttons(
                chat_id,
                player.paused.get(chat_id, False),
                player.loop.get(chat_id, False)
            )
        )
    ...
    while True:
        ...
        for chat_id in list(player.current.keys()):
            if not player.current.get(chat_id) and not player.queues.get(chat_id, []):
                try:
                    if chat_id in player.pytgcalls.active_calls:
                        await player.pytgcalls.leave_call(chat_id)
                except:
                    pass
